...
export const ProductPromoCarousel: React.FC<PromotionProps> = ({ arrowPosition }) => {
  return (
      ...
      <div className="flex gap-[12px]">
        <div className="w-[736px]">
          <div className="relative  rounded-[8px] overflow-hidden">
            <div className={`z-10 h-[calc(100%-16px)] w-[80px] right-[0px] bg-[linear-gradient(270deg,#FFFFFF_0%,rgba(255,255,255,0)_100%)] absolute rounded-r-[8px] ${arrowPosition ? (arrowPosition == 'left' ? 'left-[0px] transform scale-x-[-1]' : 'right-[0px]') : ''}`}></div>
            <CarouselProduct arrowPosition={arrowPosition}>
              {demoProducts.map((p) => (
